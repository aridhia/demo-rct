...
col_class <- function(dataset) {
  dataset <- dataset %>% mutate_if(grepl('date|day', colnames(dataset)), as.numeric) %>% #All columns containing days/dates to numeric
    mutate(a_subjectno = as.character(a_subjectno)) #Subject number as a character
  return(dataset)
}
...
#Variables to drop (not interesting)
...
baseline <- merge(subject_char, crohn_history, all = TRUE)  %>%
  #Add labels to factorial variables and generate variables 
  mutate(age = studydaydob / -365.25, #Age as years
